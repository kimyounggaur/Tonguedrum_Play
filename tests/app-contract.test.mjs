...
assert.match(html, /view-transition-name:\s*selected-drum-art/, "selected drum artwork should use a shared Card to Detail transition");
assert.match(html, /::view-transition-group\(selected-drum-art\)/, "shared artwork transition should have its own timing");
assert.match(html, /\.drum-card::before/, "drum cards should include a Motion Lab style pointer shine layer");
assert.match(html, /function updateCardMagnet/, "drum cards should support a lightweight magnetic hover interaction");
assert.match(html, /--magnet-x/, "card magnetic motion should be driven by CSS variables");
...
assert.match(html, /@keyframes tongueHitBloom/, "note play feedback should include a clipped bloom burst");
assert.match(html, /@keyframes tongueHitRing/, "note play feedback should include a clipped ring burst");
assert.match(html, /\.tongue-hit\.is-playing::before/, "tone bloom should stay on the tapped tongue hit area");
assert.match(html, /\.tongue-hit\.is-playing::after/, "tone ring should stay on the tapped tongue hit area");
assert.match(html, /@keyframes drumResonance/, "played notes should briefly resonate the drum photo");
assert.match(html, /\.drum-stage\.is-resonating \.drum-photo/, "drum resonance should be scoped to the drum stage");
assert.match(html, /--motion-enter:\s*cubic-bezier/, "remaster should adopt the Motion Lab motion-enter easing token");
assert.match(html, /--spring:\s*cubic-bezier/, "remaster should expose the Motion Lab spring easing token");
assert.match(html, /@keyframes drumAmbientGlow/, "the active drum should breathe with an ambient resonance glow");
assert.match(html, /\.drum-stage::before/, "ambient drum glow should be a stage-scoped layer behind the photo");
assert.match(html, /animation:\s*drumAmbientGlow/, "ambient drum glow keyframes should be applied to the stage");
assert.match(html, /@keyframes selectRise/, "the select screen should reveal with a staggered entrance");
assert.match(html, /#app\.is-boot-reveal \.drum-card/, "first load should run a one-shot staggered card entrance");
assert.match(html, /class="is-boot-reveal"/, "app root should start in the boot-reveal state for the entrance");
assert.match(html, /classList\.remove\("is-boot-reveal"\)/, "the entrance reveal should be a one-shot that cleans itself up");
assert.match(html, /--parallax-x/, "the decorative background should support pointer parallax depth");
assert.match(html, /\.bg-animation[\s\S]*?translate3d\(var\(--parallax-x/, "background parallax should be driven by CSS variables");
assert.match(html, /function initBackgroundParallax\(\)\s*\{[\s\S]*?shouldReduceMotion\(\)/, "background parallax should bail out under reduced motion");
assert.match(html, /pointerType === "touch"/, "pointer parallax should stay disabled for touch input");
assert.doesNotMatch(html, /\.drum-stage[^{,]*\.is-(?:playing|resonating)[^{,]*::before/, "the ambient stage glow must stay idle, not become disguised stage-level strike feedback");
assert.match(html, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?animation:\s*none/, "reduced motion should fully stop infinite ambient animations, not just shorten them");
